...
export class BackendStack extends NestedStack {
  ...
  constructor(scope: Construct, id: string, props?: NestedStackProps) {
    ...
    this.getBookUrl = this.initializeGetBookFunction(
      functionConfiguration,
      bookTable
    );
    this.getBooksUrl = this.initializeGetBooksFunction(
      functionConfiguration,
      bookTable
    );
    this.createBookUrl = this.initializeCreateBookFunction(
      functionConfiguration,
      bookTable
    );
    this.updateBookUrl = this.initializeUpdateBookFunction(
      functionConfiguration,
      bookTable
    );
    this.deleteBookUrl = this.initializeDeleteBookFunction(
      functionConfiguration,
      bookTable
    );
  }
   ...
   * @private
   */
  private initializeGetBookFunction(
   ...
   * @private
   */
  private initializeGetBooksFunction(
    functionConfiguration: NodejsFunctionProps,
    bookTable: Table
  ): FunctionUrl {
   ...
   * @private
   */
  private initializeCreateBookFunction(
    functionConfiguration: NodejsFunctionProps,
    bookTable: Table
  ): FunctionUrl {
   ...
   * @private
   */
  private initializeUpdateBookFunction(
    functionConfiguration: NodejsFunctionProps,
    bookTable: Table
  ): FunctionUrl {
   ...
   * @private
   */
  private initializeDeleteBookFunction(
    functionConfiguration: NodejsFunctionProps,
    bookTable: Table
  ): FunctionUrl {
  ...
  private outputUrl(functionUrl: FunctionUrl, name: string) {
    ...
    this.writeParameter(
      `/books/${name}Url`,
      functionUrl.url,
      `URL for ${name} function`
    );
    this.writeParameter(
      `/books/${name}Arn`,
      functionUrl.functionArn,
      `ARN for ${name} function`
    );
  }
  ...
  private writeParameter(
    ...
    new AwsCustomResource(this, parameter.replace("/", ""), {
      ...
      policy: AwsCustomResourcePolicy.fromSdkCalls({
        resources: [
          `arn:aws:ssm:us-east-1:${this.account}:parameter${parameter}`,
        ],
      }),
