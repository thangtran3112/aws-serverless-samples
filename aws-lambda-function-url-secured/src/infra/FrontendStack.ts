...
export class FrontendStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: StackProps) {
    ...
    this.distribution.addBehavior(
      "/getBook/*",
      new HttpOrigin(Fn.select(2, Fn.split("/", getBookUrl))),
      getBehaviorOptions
    );
    this.distribution.addBehavior(
      "/getBooks",
      new HttpOrigin(Fn.select(2, Fn.split("/", getBooksUrl))),
      getBehaviorOptions
    );
    ...
    this.distribution.addBehavior(
      "/createBook",
      new HttpOrigin(Fn.select(2, Fn.split("/", createBookUrl))),
      notGetBehaviorOptions
    );
    this.distribution.addBehavior(
      "/updateBook/*",
      new HttpOrigin(Fn.select(2, Fn.split("/", updateBookUrl))),
      notGetBehaviorOptions
    );
    this.distribution.addBehavior(
      "/deleteBook/*",
      new HttpOrigin(Fn.select(2, Fn.split("/", deleteBookUrl))),
      notGetBehaviorOptions
    );
              ...
              tryBundle(outputDir: string) {
                try {
                  ...
                  childProcess.execSync(
                    "cd src/front/node_modules || (cd src/front && npm ci)",
                    execOptions
                  );
                  childProcess.execSync(
                    "cd src/front && npm run build --passWithNoTests",
                    execOptions
                  );
    ...
    new CfnOutput(this, "CreateBookURL", {
      value: `https://${this.distribution.distributionDomainName}/createBook`,
    });
    new CfnOutput(this, "GetBooksURL", {
      value: `https://${this.distribution.distributionDomainName}/getBooks`,
    });
    new CfnOutput(this, "GetBookURL", {
      value: `https://${this.distribution.distributionDomainName}/getBook/id`,
    });
    new CfnOutput(this, "UpdateBookURL", {
      value: `https://${this.distribution.distributionDomainName}/updateBook/id`,
    });
    new CfnOutput(this, "DeleteBookURL", {
      value: `https://${this.distribution.distributionDomainName}/deleteBook/id`,
    });
  }
  ...
  private getParameter(parameter: string): string {
    return StringParameter.fromStringParameterName(
      this,
      parameter.toLowerCase().replace("/", "") + "Parameter",
      parameter
    ).stringValue;
  }
}
